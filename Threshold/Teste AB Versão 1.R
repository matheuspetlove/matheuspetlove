...
uf_nova_selecionada <- 'São Paulo'

localidade_selecionada <- 'Capital'
...
#### Agora vamos calculadoras o novo valor de receita e cash_margin baseado
...
df_entre_thresholds %>%
  select(id_pedido,segmento_receita,receita_bruta_pedido,`Nova Receita`,margem_2_ajustada,`Novo Cash Margin`,grupo) %>% View()
...
df_final <- df_entre_thresholds %>% group_by(uf_nova,localidade) %>%
  ...
  mutate('Diferença Receita do Intervalo (%)' = round(`Diferença Total da Receita`/`Total Receita do Intervalo Antes` * 100,2),
         'Diferença Receita Ecomm (%)' = round(`Diferença Total da Receita`/receita_total * 100,2),
         'Diferença Cash Margin do Intervalo (%)' = round(`Diferença Cash Margin`/abs(`Total Cash Margin do Intervalo Antes`) * 100,2),
         'Diferença Cash Margin Ecommerce (%)' = round((`Diferença Cash Margin`)/abs(cash_margin_total) * 100,2)) 
...
ggplotly(
  df %>%
    ...
    labs(title = "Histograma com Linhas Verticais para Thresholds",
         x = "Valor",
         y = "Contagem",
         color = "Legenda") + geom_segment(aes(x=threshold_praticado,xend=novo_threshold,y=y_barra,yend=y_barra))+
